...
void setup(){
  ...
  HatenaBox1 = new Hotpoint(200,300,1500,300);
  HatenaBox2 = new Hotpoint(-200,300,1500,300);
}
...
void draw(){
  ...
  if(userList.size()>0){
    ...
    if(kinect.isTrackingSkeleton(userId)){
      ...
      HatenaBox1.check(rightHand);
      HatenaBox1.check(leftHand);
      HatenaBox2.check(rightHand);
      HatenaBox2.check(leftHand);
      if(HatenaBox1.isHit() || HatenaBox2.isHit()){
        myPort.write("1\r\n");
      }
      else{
        myPort.write("0\r\n");
      ...
      */
      ...
      pushMatrix();
        lights();
        translate(rightHand.x,rightHand.y+magnitude/2+20,rightHand.z);
