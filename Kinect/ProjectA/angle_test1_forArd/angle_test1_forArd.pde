...
void draw(){
  ...
  if(userList.size()>0){
    ...
    if(kinect.isTrackingSkeleton(userId)){
      ...
      /*
      float ElbowCos = EtoS.dot(EtoH)/(EtoS.mag() * EtoH.mag());
      float ElbowAnglef = degrees(acos(ShoulderCos));
      int ElbowAngle = int(ElbowAnglef);
      println(ElbowAngle);
      */
      ...
      int ShoulderAngle1 = get_angle(StandardVector1, ConvertedrightElbow);
